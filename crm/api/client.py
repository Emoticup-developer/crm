...
def reset_password(request, pk):
    try:
        ...
        return Response(
            {"error": f"Client not found {ex}"}, status=status.HTTP_404_NOT_FOUND
        )

@api_view(["POST"])
def get_user(request):
    # Check for valid input
    username = request.data.get("username")  # Works for application/json
    print(username)
    if not username:
        return Response({"error": "Username is required."}, status=status.HTTP_400_BAD_REQUEST)
    
    # Fetch the client object
    client = Client.objects.filter(username=username.strip()).first()
    if not client:
